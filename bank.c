          ...
          int numWorkers)
{
  ...
  sem_init(&bank->tomorrow_lock, 0, 0);
  pthread_mutex_init(&bank->transform_lock, NULL);
  pthread_mutex_init(&bank->doreport_lock, NULL);
  pthread_mutex_init(&bank->money_lock, NULL);
  return bank;
}
