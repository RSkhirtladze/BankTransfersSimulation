...
{
  pthread_mutex_lock(&bank->transform_lock);
  ...
  if (amountAbs < reportingAmount) {
    pthread_mutex_unlock(&bank->transform_lock);
...
{
  pthread_mutex_lock(&bank->doreport_lock);
  bank->counter--;
  ...
  if(bank->counter == 0){
    ...
    if (rpt->numReports >= MAX_NUM_REPORTS) {
        // We've run out of report storage for the bank
        for(int i = 0 ; i < bank->numWorkers-1; i++)sem_post(&bank->tomorrow_lock);
        bank->counter = bank->numWorkers;
        pthread_mutex_unlock(&bank->doreport_lock);
        return -1;
    }
